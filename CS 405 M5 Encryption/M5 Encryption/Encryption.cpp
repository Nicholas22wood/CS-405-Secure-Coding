...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
  ...
  { 
      ...
      //transforming each source input using the encryption key
...
std::string read_file(const std::string& filename)
{
  ...
  // implement loading the file into a string using ifstream
  std::ifstream file(filename);

  //changing file_text to read the file that got created in the previos command
  file_text = ((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //  Line 4+: data
    ...
    //opening file to write information to it
    std::ofstream file(filename);
    
    //adding student name to the file
    file << "Student Name: " << student_name << "\n";

    //current date and time based on system
    time_t now = time(0);

    //converting now parameter to string form
    tm* ltm = localtime(&now);

    //adding timestap to file
    file << 1900 + ltm->tm_year << "-" << 1 + ltm->tm_mon << "-" << ltm->tm_mday << "\n";

    //adding key used to the file
    file << "Key used: "<< key << "\n";
